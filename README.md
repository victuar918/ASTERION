...
16. [AI 에이전트 워크플로우 로드맵](#16-ai-에이전트-워크플로우-로드맵)
...
| 제작 워크플로 | 분석·메모·BTR → Structure Type 지정 → 원석 선택 → 디자인 → 제품 이미지 → 북클릿 → 발송 |
...
| Analysis | 분석 내용 | 유지 |
| Memo | 내부 메모 (타임스탬프 추가 방식, 발송 이력 포함) | 유지 |
...
> **ExpireDate 도래 시 삭제 항목 전체**: GuestName / ForwardingDate / DeliveryCompletedDate / TempPDF(Drive 파일 포함) / ExpireDate 컬럼값 / SignReg·EvReg·PvReg 해당 StructureCode 행 전체. Memo에 발송·배송 날짜 이력이 보존되므로 컬럼 삭제 후에도 확인 가능.
...
#### analysismemo.html — 분석·메모·BTR
- **Internal Memo**: 타임스탬프 추가 방식 (덮어쓰기 없음). 영역 살짝 축소 (flex: 0.6)
- **BTR** (Birth Time Rectification): Memo와 Analysis 사이 배치
  - 12자리 숫자 입력 (예: `197911070525`)
  - 저장 형식: `1979년 11월 07일 05시 25분` (텍스트)
  - 셀 형식 텍스트(`@`) 강제 지정 → Google Sheets 날짜 자동변환 방지
  - 실시간 프리뷰 표시 (입력 즉시 변환 결과 확인)
  - 기존 값 있으면 입력란에 12자리 숫자로 복원
  - Save 시 덮어쓰기
  - ExpireDate 이후에도 영구 보존 (가공 정보, 단독으로 개인 특정 불가)
  - 북클릿 템플릿 플레이스홀더: `{{BTR}}`
- **Analysis**: 전체 덮어쓰기 방식. 영역 크기 유지 (flex: 1)
...
- 좌측: 페이지 버튼 11개 / 우측: 미리보기 영역
- Create PDF → 북클릿 PDF 폴더에 저장 → `/view` URL로 열기 (폰 자동 저장 방지)
- **TempPDF 저장 안 함** — 북클릿용과 제작용 완전 분리

#### forwarding.html — 발송 관리
- StructureCode는 worksdesk.html에서 선택 후 URL로 전달 (화면 내 별도 입력 없음)
- ForwardingDate 입력 + Update → 시트 저장, Memo 이력 추가, Status: `Forwarding`
- DeliveryCompletedDate 입력 + Update → 시트 저장, Memo 이력 추가, ExpireDate +15일 계산, Status: `Complete`
- **날짜 컬럼 초기화 시점**: Update 버튼 시 아님 → `deleteExpiredData()` 실행 시 (Memo에 이력 보존)
...
- 미생성 행 선택 → **Create Code** 버튼 → 코드 발급 + Archive 행 생성 + 제작용 PDF 생성 + TempPDF 저장
- 기존 코드 있는 행 선택 → **Create PDF** 버튼 → 제작용 PDF 재생성
- 용도: 시스템 오류로 StructureCode 미생성 시 구글폼 재접수 없이 전 과정 복구 (고객의 법적 동의 시점 보전)

#### inventory.html, stoneinfo.html, invenmanage.html, newlisting.html
- 원석 목록 조회·관리 / 재고 수량 업데이트 / 원석 이미지 업로드 / 신규 원석 등록
...
var ORDER_PDF_FOLDER_ID = "117dfqwOb1VuF_Cz...";   // 제작용 PDF (별도 폴더)
...
- 원석 수 기준 템플릿 자동 선택
- 플레이스홀더 치환 후 PDF 변환
- **PDF_FOLDER_ID** (북클릿 전용 폴더)에 저장
- **TempPDF 저장 안 함** — 북클릿용과 제작용 완전 분리
- 반환 URL: `/view` 형식 (자동 다운로드 방지)

#### `createOrderPdf(structureCode)` — 제작용 PDF 생성
- SignReg 시트에서 폼 원본 데이터(성함·생년월일·출생시간·연락처·배송주소 등) 조회
- Google Docs로 제작 신청서 생성 후 PDF 변환
- **ORDER_PDF_FOLDER_ID** (제작용 전용 폴더)에 저장
- Archive 시트 `TempPDF` 컬럼에 파일 ID 저장 (ExpireDate 기준 삭제 추적)
- 반환 URL: `/view` 형식 (자동 다운로드 방지)
- neworder.html의 Create Code / Create PDF 버튼 전용

#### `recreatePdf(structureCode)` — 제작용 PDF 재생성
- 기존 TempPDF Drive 파일을 먼저 삭제 (TempPDF 컬럼값 공백 처리)
- `createOrderPdf()` 재호출 → 새 파일 생성 + TempPDF 컬럼값 갱신

#### `_removeIfPhaseBlock(body, removeBlock)` — {{#IF_PHASE}} 처리
```
템플릿 형식 (반드시 한 줄 인라인):
  {{#IF_PHASE}}Structure 유효 구간{{/IF_PHASE}}
  {{#IF_PHASE}}{{PhaseStartDate}} ~ {{PhaseEndDate}}{{/IF_PHASE}}

removeBlock=true  (Core Structure / 미지정):
  태그를 포함하는 단락 전체를 역순으로 삭제

removeBlock=false (Phase Structure):
  body.replaceText()로 태그 텍스트만 제거, 내용 유지
```

> **주의**: 태그가 서로 다른 단락에 있으면 처리 불가. 반드시 같은 줄 인라인으로 작성.

#### `updateAnalysisMemo(payload)` — 분석·BTR 저장
- `payload.analysis`: 덮어쓰기
- `payload.btr`: `setNumberFormat("@")`으로 셀 형식 텍스트 강제 후 저장 (날짜 자동변환 방지)
- `payload.memo`: 선택적 덮어쓰기

#### `updateForwarding(payload)` — 발송 정보 저장
- ForwardingDate만 입력: 시트 저장 + Memo 이력 추가 + Status: `Forwarding`
- 두 날짜 모두 입력: 시트 저장 + Memo 이력 추가 + ExpireDate(+15일) 계산 + Status: `Complete`
- **날짜 컬럼 초기화는 이 함수가 아닌 `deleteExpiredData()`에서 처리**

#### `deleteExpiredData()` — 만료 데이터 삭제
- 기준: Archive 시트 `ExpireDate` ≤ 오늘 (매일 03:00 자동 실행)
- 삭제 항목 (순서대로):
  1. Archive `GuestName` 컬럼값
  2. Archive `ForwardingDate` 컬럼값
  3. Archive `DeliveryCompletedDate` 컬럼값
  4. Archive `TempPDF` 파일 ID로 Drive 파일 영구 삭제 → 컬럼값 공백
  5. `ExpireDate` 컬럼값 (재실행 방지)
  6. SignReg/EvReg/PvReg 해당 StructureCode 행 전체 삭제
...
- `createOrderPdf(code)` 자동 호출 → 제작용 PDF 생성 + TempPDF 저장
...
deleteExpiredData()
  ...
  삭제:
    ...
    - Archive ForwardingDate 컬럼값
    - Archive DeliveryCompletedDate 컬럼값
    - TempPDF Drive 파일 (파일 ID로 직접 삭제) + 컬럼값
...
| 내용 | StructureCode + 폼 원본 답변 전체 | 고객용 11페이지 북클릿 |
| 저장 폴더 | ORDER_PDF_FOLDER_ID | PDF_FOLDER_ID |
| TempPDF 기록 | ✅ (삭제 추적) | ❌ |
| ExpireDate 삭제 | ✅ | — |
| 반환 URL | `/view` 형식 | `/view` 형식 |

### 제작용 PDF 내용 구성

| 항목 | 내용 |
|------|------|
| Structure Code | 발급된 StructureCode |
| 접수 시트 | SignReg / EvReg / PvReg |
| 접수일시 | 타임스탬프 또는 Created Date |
| 성함 | 폼 응답의 성함 |
| 생년월일 | 폼 응답의 생년월일 |
| 출생시간 | 폼 응답의 출생시간 |
| 연락처 | 폼 응답의 연락처 |
| 배송주소 | 폼 응답의 배송주소 |
| 추가 정보 | EvReg 전용: 기존 Archive No. 등 추가 폼 항목 |
...
- 템플릿에서 **반드시 같은 줄 인라인**으로 작성:
  ```
  {{#IF_PHASE}}내용{{/IF_PHASE}}
  ```
- `PhaseStartDate` 있음 (Phase Structure): 태그만 제거, 내용 유지
- `PhaseStartDate` 없음 (Core Structure / 미지정): 태그 포함 단락 전체 삭제
...
### ExpireDate 자동 계산 및 삭제 흐름

```
DeliveryCompletedDate 입력 + Update
    ↓
...
② Archive ForwardingDate 삭제
③ Archive DeliveryCompletedDate 삭제
④ TempPDF Drive 파일 삭제 + 컬럼값 삭제
⑤ ExpireDate 컬럼값 삭제 (재실행 방지)
...
| ForwardingDate 입력 + Update | 시트 저장 + Memo 이력 추가 + Status: Forwarding |
| DeliveryCompletedDate 입력 + Update | 시트 저장 + Memo 이력 추가 + ExpireDate +15일 계산 + Status: Complete |
| ExpireDate 도래 (deleteExpiredData) | ForwardingDate + DeliveryCompletedDate 컬럼값 삭제 |
| 삭제 이후 | Memo에 이력 텍스트 영구 보존 — 날짜 삭제 후에도 발송·배송 기록 확인 가능 |

### A/S 재진입 흐름

1. `structureindex.html`에서 StructureCode 검색 → Status: Complete → **A/S**로 변경
2. `worksdesk.html`에서 해당 항목 확인 가능 (A/S는 표시 대상 Status)
3. `forwarding.html`에서 새 ForwardingDate 입력 → Status: **Forwarding**
4. 배송 완료 후 DeliveryCompletedDate 입력 → ExpireDate 갱신 → Status: **Complete**
5. A/S 전용 별도 처리 없음. 동일 프로세스 반복
...
- **BTR** (가공된 출생 시각 — 단독으로 개인 특정 불가, 데이터 누적 시 특정 가능성 더욱 낮아짐)
- Analysis, Memo (발송 날짜 이력 포함), UsedStones, LayoutSummary, DetailsStr
- ProductImage
...
Google Drive
├── 북클릿 PDF 폴더 (1w0Ll81Al6sFVLtG3X5vHdL_QYryfpTST)
│     └── S-00001AA-260410_1744200000000.pdf
│
├── 제작용 PDF 폴더 (117dfqwOb1VuF_CzCWpcT50baBrgzJTrZ)
│     └── ORDER_S-00001AA-260410_20260410_123456.pdf
│
├── 제품 이미지 폴더 (19-712Gf2gX3YLP23JvcQ_snUbhGByJ9v)
│     └── S00001AA2604101744200000000.jpg
│
...
- `RegisterDate` 컬럼 삭제 → `Created Date`로 통합
- `ProductImageUrl` 컬럼 삭제 → `ProductImage`로 통합
- `BirthDate`, `BirthTime`, `Phone`, `Address` 컬럼 삭제 (Archive에 저장 불필요)
...
- 여러 단락에 걸친 블록 치환 불가
- 해결: `{{#IF_PHASE}}내용{{/IF_PHASE}}` 형식으로 반드시 한 줄 인라인 작성
...
> 현재 Archive 시트에 구형 코드와 신형 코드가 혼재할 수 있음. 시스템은 두 형식 모두 조회 가능.

---

## 16. AI 에이전트 워크플로우 로드맵

### 개요

Gemini(분석관)와 Claude(검증관)의 역할 분리 기반 상호 검증으로 BTR 신뢰성을 확보하는 지능형 분석 엔진. **Phase 2 이후 적용 예정.**

### AI 역할 정의

| AI | 역할 | 주요 업무 |
|----|------|----------|
| Gemini | 분석관 | 베딕 점성술 관점 BTR 후보 시각 도출, 차트 기반 근거 서술 |
| Claude | 검증관·채점관 | 루브릭 기준 독립 채점, 이견 항목 구체적 근거 명시 |

> 채점 원칙: Claude는 "99점 달성을 목표로" 채점하지 않음. 루브릭 기준으로 항목별 독립 평가.

### Archive 시트 추가 컬럼 (Phase 2 시)

| 컬럼명 | 내용 |
|--------|------|
| BTRStatus | Pending / InProgress / Confirmed / Held |
| BTRIteration | 현재 루프 횟수 (1~5) |
| GemAnalysis1~5 | 각 회차 Gemini 분석 Docs 파일 ID |
| ClAnalysis1~5 | 각 회차 Claude 검증 Docs 파일 ID |
| AnalysisScore | 최종 채점 점수 |
| AnalysisDocs | 최종 문서 파일 ID |

### AnalysisScore 루브릭 (100점 만점, 99점 이상 확정)

| 평가 항목 | 배점 | 비고 |
|----------|------|------|
| 과거 사건 부합성 | 40점 | 사건 3건 미만 시 25점 초과 불가 |
| 나바암샤(D-9) 정렬 | 20점 | 라시↔나바암샤 논리 모순 없을 때 만점 |
| 외형 및 기질 일치도 | 15점 | 기질 정보 미제공 시 10점 초과 불가 |
| 다샤 전환점(Sandhi) 검증 | 15점 | 인생 변곡점과 샌디 구간 일치도 |
| 논리 일관성 보너스 | 10점 | 두 AI 핵심 결론 일치 시 보너스 부여 |

### AnalysisDocs 상태별 구분

| BTRStatus | AnalysisDocs 내용 | 알람 |
|-----------|------------------|------|
| Confirmed | 전문가 분석 보고서 (Internal Report) | 없음 |
| Held | BTR 분석 보류 보고서 | **이메일 즉시 발송** |

### 파일 ID 기반 추적 및 자동 삭제

- 분석 과정 생성 임시 Docs → GemAnalysis/ClAnalysis 컬럼에 파일 ID 기록
- 99점 확정(Confirmed) 시: 임시 Docs 전부 Drive에서 영구 삭제 → 컬럼값 공백 처리
- 5회 초과(Held) 시: 기존 파일 Backup_Archive 폴더로 격리 이동, AnalysisDocs에 보류 보고서 ID 기록
- 리셋 실행 시: Backup 파일 삭제 → 컬럼 초기화 → Pre-Check부터 재시작

### 단계별 구현 로드맵

| Phase | 시점 | 주요 내용 |
|-------|------|----------|
| Phase 1 | 현재 | Archive System 기본 운영. BTR 수동 입력 |
| Phase 2 | 월 2~3건 안정화 | Gemini-Claude 순차 호출, 파일 ID 기반 임시 Docs 자동 삭제, Held 이메일 알람, 재고 임계치 알람 |
| Phase 3 | 성공 사례 50건 이상 | 비식별화 사례 기반 루브릭 보정, 분석 정밀도 강화, 공급처 선정 알고리즘 |

### 구현 기술 기반

| 기능 | 구현 방식 |
|------|----------|
| Gemini API 호출 | UrlFetchApp → Vertex AI / Gemini API |
| Claude API 호출 | UrlFetchApp → Anthropic API |
| Docs 생성 + 파일 ID 저장 | DocumentApp + `_ensureColumn()` 패턴 |
| 파일 ID 기반 일괄 삭제 | `DriveApp.getFileById().setTrashed()` |
| Backup 폴더 이동 | `DriveApp.getFileById().moveTo()` |
| 이메일 알람 | `MailApp.sendEmail()` |
| 재고 임계치 감지 | Inventory 시트 스캔 + 시간 기반 트리거 |
