...
data class VoiceConfig(val speaker1: SpeakerConfig, val speaker2: SpeakerConfig, val speaker3: SpeakerConfig) {
...
class SupertonicTtsEngine(private val context: Context) {
    ...
    suspend fun init(onProgress: (String) -> Unit = {}) = withContext(Dispatchers.IO) {
        ...
        val config = OfflineTtsConfig(
            model = OfflineTtsModelConfig(
                vits = OfflineTtsVitsModelConfig(model="$dir/model.onnx", lexicon="$dir/lexicon.txt", tokens="$dir/tokens.txt", dataDir="$dir/espeak-ng-data"),
                numThreads=4, debug=false, provider="cpu"
            ), ruleFsts="", maxNumSentences=1
        )
        tts = OfflineTts(config)
        onProgress("✅ TTS 준비 완료")
    }

    fun synthesize(text: String, sid: Int, speed: Float, outputFile: File): Boolean {
        if (text.isBlank() || tts == null) return false
        return try {
            val audio = tts!!.generateWithCallbackAndNumberOfThreads(text=text, sid=sid, speed=speed, callback=null, numThreads=4)
            outputFile.writeBytes(float32ToWav(audio.samples, audio.sampleRate))
            true
        } catch(e: Exception) { Log.e(TAG, "TTS 실패: $e"); false }
    }
    ...
    private suspend fun prepareModelDir(onProgress: (String)->Unit): File = withContext(Dispatchers.IO) {
        ...
        if (marker.exists()) return@withContext dest
        dest.mkdirs()
    ...
    private fun downloadFile(client: OkHttpClient, url: String, dest: File) {
        client.newCall(Request.Builder().url(url).build()).execute().use { resp ->
            if (!resp.isSuccessful) throw Exception("다운로드 실패 ${resp.code()}: $url")
