...
class AsterionRenderEngine(
    ...
    suspend fun init(onProgress: (String)->Unit = {}) {
        ...
        tts.init(onProgress)
        Config.setLogLevel(com.arthenica.mobileffmpeg.Level.AV_LOG_WARNING)
    }
    ...
    suspend fun renderScene(row: ScriptDataRow, meta: VideoMeta, onProgress: (String)->Unit = {}): File? =
        withContext(Dispatchers.IO) {
            ...
            try {
                ...
                val t = if (wav.exists()) tts.estimateDurationFromFile(wav) else 3.0f
                ...
                if (rc == 0) { clips.add(out); onProgress("[$id] ✅"); out }
                else { Log.e(TAG, Config.getLastCommandOutput()); onProgress("[$id] ❌"); null }
    ...
    suspend fun concatSubclips(name: String, bgm: String, onProgress: (String)->Unit = {}): File? =
        withContext(Dispatchers.IO) {
            ...
            val bgmF = AppConfig.resolveBgm(bgm)
            val cmd  = if (bgmF != null)
                "-y -f concat -safe 0 -i ${list.absolutePath} -i ${bgmF.absolutePath} " +
            ...
            else "-y -f concat -safe 0 -i ${list.absolutePath} -c copy ${out.absolutePath}"
            val rc = FFmpeg.execute(cmd)
            list.delete()
            if (rc == 0) { onProgress("✅ ${out.name} (${out.length()/1024/1024}MB)"); out } else null
        }
    ...
    private fun buildCmd(bg: File, wav: File?, t: Float, ass: File, style: CardStyle, grad: GradientPreset, fx: String, out: File): String {
        ...
        val fxP   = if (fxF.isNotEmpty()) ",$fxF" else ""
        val cardP = if (style!=CardStyle.NONE && style!=CardStyle.MINIMAL) {
        ...
        val filter   = "[0:v]trim=duration=$t,setpts=PTS-STARTPTS${fxP}${cardP},subtitles='$ap'[v]"
        val audioIn  = if (wav!=null) "-i ${wav.absolutePath} " else ""
        val audioMap = if (wav!=null) "-map 1:a -c:a aac -b:a 192k " else "-an "
        // mobile-ffmpeg: h264_mediacodec 하드웨어 가속 사용
        return "-y -stream_loop -1 -i ${bg.absolutePath} ${audioIn}" +
